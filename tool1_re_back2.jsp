...
rc.eval("gene <- unlist(strsplit('"+gene+"',','))");

rc.eval("options(stringsAsFactors = F)");
rc.eval("data <- read.table('"+dataPath+"/web1_231110.txt', sep =',',header=TRUE)");
rc.eval("data_plot <- data[,colnames(data) %in% gene]");
rc.eval("colnames(data_plot) <- c('gene_name_1','gene_name_2')");

rc.eval("library(ggplot2)");
rc.eval("library(ggpubr)");
rc.eval("library(ggExtra)");

rc.eval("p <- ggplot(data_plot, aes(x=gene_name_1, y=gene_name_2)) +geom_point(colour = '#4DBBD5CC',alpha = 0.5) +geom_smooth(method=lm , color='#DC0000FF', fill='#F39B7FCC', se=TRUE) +ylab(gene[3])+ xlab(gene[2]) + theme_bw()+stat_cor(method = 'pearson',label.x = 0.35,color= '#DC0000FF')");
